...
CREATE TABLE sessions (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), owner_id uuid NOT NULL REFERENCES auth.users(id), experience_id uuid NOT NULL REFERENCES experiences(id) ON DELETE CASCADE, status text NOT NULL, started_at timestamptz NOT NULL DEFAULT now(), ended_at timestamptz);
...
BEGIN
  CASE TG_TABLE_NAME
    ...
    WHEN 'experience_items' THEN SELECT EXISTS (SELECT 1 FROM experiences e JOIN fragments f ON true WHERE e.id = NEW.experience_id AND f.id = NEW.fragment_id AND e.owner_id = NEW.owner_id AND f.owner_id = NEW.owner_id) INTO parent_owned;
    WHEN 'sessions' THEN SELECT EXISTS (SELECT 1 FROM experiences e WHERE e.id = NEW.experience_id AND e.owner_id = NEW.owner_id) INTO parent_owned;
    WHEN 'exposures' THEN SELECT EXISTS (SELECT 1 FROM sessions s JOIN experience_items i ON true WHERE s.id = NEW.session_id AND i.id = NEW.experience_item_id AND s.owner_id = NEW.owner_id AND i.owner_id = NEW.owner_id) INTO parent_owned;
