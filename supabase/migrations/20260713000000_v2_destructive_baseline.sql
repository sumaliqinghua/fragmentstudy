...
CREATE TABLE sessions (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), owner_id uuid NOT NULL REFERENCES auth.users(id), project_id uuid NOT NULL REFERENCES projects(id) ON DELETE CASCADE, project_material_id uuid NOT NULL REFERENCES project_materials(id) ON DELETE CASCADE, experience_id uuid NOT NULL REFERENCES experiences(id) ON DELETE CASCADE, mode text NOT NULL, status text NOT NULL, current_item_index integer NOT NULL DEFAULT 0 CHECK (current_item_index >= 0), started_at timestamptz NOT NULL DEFAULT now(), ended_at timestamptz, end_reason text);
