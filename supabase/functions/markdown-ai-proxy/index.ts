...
} from '../_shared/markdownReaderAi.ts';
import {
  extractBearerToken,
...
Deno.serve(async (request: Request) => {
  ...
  if (request.method !== 'POST') {
    return jsonResponse(405, { error: '仅支持 POST 请求' }, corsHeaders);
  }

  const declaredLength = Number(request.headers.get('content-length') || 0);
  if (Number.isFinite(declaredLength) && declaredLength > MAX_BODY_BYTES) {
    return jsonResponse(413, { error: '请求内容过大' }, corsHeaders);
  }
  ...
  try {
    ...
    const body = validateMarkdownReaderAiRequest(parsedBody);
    ...
    if (!upstream.ok) {
      return jsonResponse(502, { error: `平台 AI 请求失败 (${upstream.status})` }, corsHeaders);
    }
    ...
    return jsonResponse(500, { error: 'AI 代理发生内部错误' }, corsHeaders);
  }
});
